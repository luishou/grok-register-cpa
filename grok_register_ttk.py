...
def validate_registration_config():
    ...
    provider = str(get_email_provider() or "cloudflare").strip().lower()
    supported = ("duckmail", "yyds", "cloudflare", "mailnest", "cloudmail")
    ...
    if provider == "mailnest" and not str(config.get("mailnest_api_key", "") or "").strip():
        raise ValueError("MailNest 模式需要先填写 mailnest_api_key")
    if provider == "cloudmail":
        ...
        if not get_cloudmail_password():
            missing.append("cloudmail_password")
        if not str(config.get("defaultDomains", "") or "").strip():
            missing.append("defaultDomains")
        if missing:
            raise ValueError(f"CloudMail 模式缺少配置: {', '.join(missing)}")
    if config.get("cpa_auto_add") and not config.get("cpa_auth_dir") and not config.get("cpa_remote_url"):
        raise ValueError("已开启 SSO→auth，但未配置 cpa_auth_dir 或 cpa_remote_url")
